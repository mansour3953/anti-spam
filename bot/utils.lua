...
tdcli_function ({
  ...
  if not lang then
    ...
    return tdcli.sendMessage(msg.to.id, msg.id, 0, "_ „«„ «œ„?‰ Â«? ê—ÊÂ »Â „ﬁ«„ „œ?— „‰ ’» ‘œ‰œ Ê ”«“‰œÂ ê—ÊÂ »Â „ﬁ«„ „«·ò ê—ÊÂ „‰ ’» ‘œ_", 0, "md")
     end
 end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['banned']) == nil then --fix way
     if not lang then
					...
					return "*Â?ç ò«—»—? «“ «?‰ ê—ÊÂ „Õ—Ê„ ‰‘œÂ*"
              end
				end
       if not lang then
   message = '*List of banned users :*\n'
         else
   message = '_·?”  ò«—»—«‰ „Õ—Ê„ ‘œÂ «“ ê—ÊÂ :_\n'
     end
...
local lang = redis:get(hash)
  ...
  if next(data['gban_users']) == nil then --fix way
    if not lang then
					...
					return "*Â?ç ò«—»—? «“ ê—ÊÂ Â«? —»«  „Õ—Ê„ ‰‘œÂ*"
             end
				end
        if not lang then
   message = '*List of globally banned users :*\n'
   else
   message = '_·?”  ò«—»—«‰ „Õ—Ê„ ‘œÂ «“ ê—ÊÂ Â«? —»«  :_\n'
   end
